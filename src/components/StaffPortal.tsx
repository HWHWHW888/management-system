...
import { CalendarDays, Clock, Users, Eye, EyeOff, Plus, Trash2, Edit, Save, X, CheckCircle, XCircle, Upload, FileText, AlertTriangle, RefreshCw, UserPlus, UserMinus, LogIn, LogOut, Shield, Key, Settings, Activity, MapPin, Phone, Mail, Badge as BadgeIcon, IdCard, Briefcase, AlertCircle, Database, Info, Calendar, BarChart3 } from 'lucide-react';
...
function StaffPortalComponent({ user, showError, clearError }: StaffPortalProps) {
  ...
  const loadRealTimeData = useCallback(async () => {
    try {
      ...
      showError(`Failed to load staff portal data: ${errorMessage}`);
    } finally {
      setIsLoadingData(false);
    }
  }, [user.staffId, user.username, clearError, showError]);
  ...
  const saveShiftsToSupabase = async (updatedShifts: StaffShift[]) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
      showError(`Failed to save shifts data: ${errorMessage}`);
    }
  };
