...
function CustomerManagementComponent({ user, showError, clearError }: CustomerManagementProps) {
  ...
  const loadRealTimeData = useCallback(async () => {
    try {
      ...
      const processedCustomers = customersData.map((customer: Customer) => {
        ...
        return {
          ...customer,
          // Preserve extended customer details
      ...
      const processedTrips = tripsData.map((trip: Trip) => ({
        ...trip,
        totalRollingPercentage: trip.totalRollingPercentage || 1.4,
        calculatedTotalRolling: trip.calculatedTotalRolling || (trip.totalRolling || 0) * 0.014,
        ...
        customers: trip.customers?.map(customer => {
          ...
          const tripRollingRecords = rollingData.filter(record => 
            ...
            record.recordedAt <= (trip.endDate || new Date().toISOString())
          );
          
          const realTimeTripWinLoss = tripRollingRecords.reduce((sum, record) => sum + (record.winLoss || 0), 0);
  ...
  const saveCustomersToSupabase = async (updatedCustomers: Customer[]) => {
    try {
      ...
      showError(`Failed to save customer data: ${error.message}`);
      throw error;
    } finally {
      setSaving(false);
    }
    ...
    <div className="space-y-6">
      ...
      <Alert>
        ...
        <AlertDescription>
          ...
          full customer profile editing with passport/ID/preferences, and live trip history updates.
        </AlertDescription>
      </Alert>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <div className="space-y-4">
                              ...
                              <div>
                                ...
                                <p>{customer.passportNumber || 'Not provided'}</p>
                              </div>
                              <div>
                                <Label className="text-sm font-medium text-gray-500">ID Number</Label>
                                <p>{customer.idNumber || 'Not provided'}</p>
                              </div>
                              <div>
                                <Label className="text-sm font-medium text-gray-500">Nationality</Label>
                                <p className="flex items-center">
                                  {customer.nationality && <Globe className="w-4 h-4 mr-1" />}
                              ...
                              <div>
                                ...
                                <p className="flex items-center">
                                  {customer.dateOfBirth && <Cake className="w-4 h-4 mr-1" />}
                              ...
                              <div>
                                ...
                                <p className="flex items-center">
                                  {customer.address && <Home className="w-4 h-4 mr-1" />}
                                  {customer.address || 'Not provided'}
                                </p>
                              </div>
                              <div>
                                <Label className="text-sm font-medium text-gray-500">Occupation</Label>
                                <p>{customer.occupation || 'Not provided'}</p>
                              </div>
                            </div>
      ...
      <Dialog open={isDetailDialogOpen} onOpenChange={setIsDetailDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Update comprehensive customer information including identity, preferences, and financial details
            </DialogDescription>
          </DialogHeader>
            ...
            <Tabs defaultValue="identity" className="w-full">
              ...
              <TabsContent value="preferences" className="space-y-4 mt-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    <Label htmlFor="preferences">Gaming Preferences</Label>
                    <Textarea
                      id="preferences"
                      value={detailFormData.preferences}
                      onChange={(e) => setDetailFormData({...detailFormData, preferences: e.target.value})}
                      placeholder="Enter gaming preferences, favorite games, etc."
                      rows={3}
                      disabled={saving}
                    />
                  </div>
