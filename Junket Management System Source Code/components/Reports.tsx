...
export function Reports({ user }: ReportsProps) {
  ...
  const loadRealTimeReportsData = useCallback(async () => {
    try {
      ...
      setErrorMessage(`Failed to load reports data: ${error.message}`);
  ...
  const getFilteredData = () => {
    ...
    if (user.role === 'agent' && user.agentId) {
      ...
      const staffCustomerIds = [...new Set([...filteredRollingRecords.map(r => r.customerId), ...filteredBuyInOutRecords.map(b => b.customerId)])];
