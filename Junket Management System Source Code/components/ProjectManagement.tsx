...
function ProjectManagementComponent({ user, showError, clearError }: ProjectManagementProps) {
  ...
  const loadAllRealTimeData = useCallback(async () => {
    try {
      ...
      const errorMsg = `Failed to load project data: ${error.message}`;
  ...
  const saveTrips = async (updatedTrips: Trip[]) => {
    try {
      ...
      showError(`Failed to save trips: ${error.message}`);
      throw error;
    } finally {
      setSaving(false);
    }
  ...
  const handleAddCustomerToTrip = async (customerId: string) => {
    ...
    try {
      ...
      showError(`Failed to add customer to trip: ${error.message}`);
    }
  };
  ...
  const handleRemoveCustomerFromTrip = async (customerId: string) => {
    ...
    try {
      ...
      showError(`Failed to remove customer from trip: ${error.message}`);
    }
  };
  ...
  const handleAddAgentToTrip = async (agentId: string) => {
    ...
    try {
      ...
      showError(`Failed to add agent to trip: ${error.message}`);
    }
  };
  ...
  const handleEditAgentShare = async () => {
    ...
    try {
      ...
      showError(`Failed to update agent share: ${error.message}`);
    }
  };
  ...
  const handleDeleteAgentFromTrip = async () => {
    ...
    try {
      ...
      showError(`Failed to remove agent from trip: ${error.message}`);
    }
  };
  ...
  const handleAddExpense = async () => {
    ...
    try {
      ...
      showError(`Failed to add expense to trip: ${error.message}`);
    }
  };
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <CardDescription className="mt-1">
                            {trip.description} • {trip.venue} • {trip.date}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center space-x-4 mt-2">
                    ...
                    {selectedTrip.venue && <span className="text-sm text-gray-500">{selectedTrip.venue}</span>}
                  </div>
                </div>
